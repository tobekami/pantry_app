...
import { firestore, collection, addDoc, doc, updateDoc, deleteDoc, query, where, getDocs } from '../lib/firebaseConfig';
...
const PantryForm: React.FC<PantryFormProps> = ({ item, onSubmit, onDelete, user, isNewItem = false }) => {
  ...
  const handleSubmit = () => {
    ...
    // 1. Force Quantity to be a Number
    ...
    const newItem: PantryItem = {
      ...item, // Keep ID if it exists
      name: capitalizedName,
      quantity: finalQuantity,
      userId: user.uid
    };

    // 3. Just tell the parent to save. Don't save to Firebase here.
    onSubmit(newItem);
  ...
  const handleIncrement = () => {
    ...
    // If it's an existing item, trigger save immediately
    if (!isNewItem && item) {
      onSubmit({ ...item, quantity: newQ });
    }
  };

  const handleDecrement = () => {
    const newQ = Math.max(0, Number(quantity) - 1);
    setQuantity(newQ);
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
      ...
      {!isNewItem && onDelete && (
        <IconButton onClick={() => onDelete && onDelete(item!.id!)} size="small">
